...
rm(tmp1, tmp2)
```
Use case_when() in mutate() to add a new column to a df based on a comparison 
between other columns in the df.
```{r}
gdata <- gdata %>% 
  mutate(wb_int = case_when(
    source == "WormBase" & type == "intron" ~ TRUE,
                            TRUE ~ FALSE))
```
Create a vector with all introns that are wormbase introns. Which gene each 
intron belongs to is also included.
```{r}
